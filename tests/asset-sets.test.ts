...
void test('complete design sets require review; costume references stay separate; prop states are explicit',()=>{
 ...
 assert.throws(()=>requireAssetSets(p),/整套/);
 ...
 assert.ok(!approvedAssets(p).includes(costume));assert.throws(()=>selectCostume(p,costume.id),/审核/);
