...
## 2026-09-16 · 返工闭环接通、局部修订与视觉审查加深

按第七轮评审补齐三个具体短板：原生视频提示词接入审查意见、修订粒度精确化、视觉审查深度与证据。

### 修复

- **原生返工闭环**：`reflectionDirective()` 从 QA 记录提取意见与结构化 findings，注入 MiniMax H3 与 Kling（videoText）提示词的 `返工` 字段——审查退回后的重新生成会优先按具体问题与修复逐项修正，而不是原样重发。
- **局部修订与精确失效**：`revise_shots` 支持 `targets`（只改指定镜头，其余镜头逐字段保留）；无 targets 时按镜头内容逐镜 diff，失效从**第一个实际变化的镜头**开始——修一个持刀状态不再使全套下游素材失效。未受影响镜头的参考图与视频保留。
- **视觉审查加深**：抽帧默认 5 帧 / 720px（`VISUAL_QA_FRAME_COUNT` / `VISUAL_QA_FRAME_WIDTH` 可调）；前镜改按**叙事线**取（交叉剪辑不混淆线程），附带前镜视频地址；请求携带已批准资产参考图（本地图转 data URL）与对白表演时间窗，新增 `lip_sync` 维度；声音连续性明示需音频能力评测服务核查。
- **开发服务器稳定性**：Vite 监视忽略编辑器原子替换临时目录（`**/.*.tmpdir/**`），避免编辑时 EBUSY 崩溃。

### 文档与验证

- 新增定向修订回归（未目标镜头逐字段保留 + 媒体保留、失效自首变镜头起、pendingReview 生效）；57 个测试文件、typecheck、lint、build 四 gate 全部通过。
- 网关协议文档同步帧级审查的参考图与时间窗字段。
