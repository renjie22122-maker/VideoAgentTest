...
export function AssetUpload({assetId,disabled,onUpload}:{assetId:string;disabled:boolean;onUpload:(action:string,data:Record<string,unknown>)=>Promise<unknown>}){
 ...
 async function upload(file:File){setError('');setLoading(true);try{
  ...
  await onUpload('asset_upload',{assetId,imageBase64:data,filename:file.name});
 }catch(e){setError(e instanceof Error?e.message:'图片无法读取。');}finally{setLoading(false);}}
 return <div className="asset-prompt-card"><label>或上传本地图片作为此候选<input type="file" accept="image/png,image/jpeg,image/webp" disabled={disabled||loading} onChange={e=>{const file=e.target.files?.[0];e.target.value='';if(file)void upload(file);}}/></label><p className="help">PNG / JPEG / WebP，最大 8 MB。保存到本机，上传后仍需确认使用；不会调用生图 API。</p>{loading&&<output>正在读取并保存图片…</output>}{error&&<p role="alert">{error}</p>}</div>;
}
