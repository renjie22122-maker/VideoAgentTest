...
export function AssetUpload({assetId,disabled,onUpload,shotId,actionOverride}:{actionOverride?:string;assetId:string;shotId?:string;disabled:boolean;onUpload:(action:string,data:Record<string,unknown>)=>Promise<unknown>}){
 ...
 async function upload(file:File){setError('');setLoading(true);try{
  ...
  await onUpload(actionOverride??(shotId?'shot_image_upload':'asset_upload'),{assetId,shotId,imageBase64:data,filename:file.name});
 }catch(e){setError(e instanceof Error?e.message:'图片无法读取。');}finally{setLoading(false);}}
 return <div className="asset-prompt-card"><label>{actionOverride==='video_tail_upload'?'上传本镜尾帧':shotId?'上传图片作为当前分镜画面':'或上传本地图片作为此候选'}<input type="file" accept="image/png,image/jpeg,image/webp" disabled={disabled||loading} onChange={e=>{const file=e.target.files?.[0];e.target.value='';if(file)void upload(file);}}/></label><p className="help">PNG / JPEG / WebP，最大 8 MB。{shotId?'上传后作为本镜首帧；本镜视频及后续素材失效，需重新检查并批准生成。':'保存到本机，上传后仍需确认使用。'} 不会调用生图 API。</p>{loading&&<output>正在读取并保存图片…</output>}{error&&<p role="alert">{error}</p>}</div>;
}
