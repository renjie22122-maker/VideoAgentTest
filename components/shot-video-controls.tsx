...
export function ShotVideoControls({project,shot,busy,act}:{project:Project;shot:Shot;busy:boolean;act:(action:string,data?:Record<string,unknown>)=>Promise<unknown>}){
 ...
 <p>仅更换视频输入配置会保留已完成的文字分镜检查与批准，本镜旧视频需要重新生成和审查。参考图模式最多 9 张，仅 H3 支持；生成需符合模型时长范围。以下按钮仅提交本镜，真实模式按调用计费。</p>
 <div className="queue-actions">{(['image','video'] as const).map(kind=><div key={kind}><Button disabled={busy||pending||changed||!ready} onClick={()=>void act('enqueue',{shotId:shot.id,kind})}>仅生成 / 恢复本镜{kind==='image'?'画面':'视频'}</Button><Button variant="outline" disabled={busy||pending||changed||!ready||!(kind==='image'?shot.referenceUrl:shot.videoUrl)} onClick={()=>void act('enqueue',{shotId:shot.id,kind,regenerate:true})}>重新生成本镜{kind==='image'?'画面':'视频'}（新调用）</Button></div>)}</div>
