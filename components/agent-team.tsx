...
export function AgentTeam({
  ...
  return (
    <section className="panel">
      ...
      />
      <p>
        当前阶段建议：{recommended.map((r) => r.name).join('、')}
        。各部门读取同一版剧本、资产与分镜；报告提供证据与返工岗位，由导演和用户决定修改。修改作品后旧报告需重新审查。
      </p>
      <p className="help">
        真实模式下，每次会审调用一次语言模型并计费。这里审查文字方案，不把未看过的图像或视频判为合格；不会自动启动付费生图或视频。
      </p>
      {filmTeam.map((role) => {
        const report = [...(project.production?.teamReports ?? [])]
          .reverse()
            ...
            <summary>
              ...
              {report
                ? report.revision === project.revision &&
                  (report.configRevision ?? 0) ===
                    (project.production?.agentConfigRevision ?? 0)
                  ? '本版已有报告'
                  : '旧版报告待更新'
                : '尚未独立会审'}
            </summary>
            <p>交付：{role.deliverable}</p>
            <p>{role.checks}</p>
            <Button
              disabled={busy || !role.enabled}
              onClick={() => void act('team_review', { roleId: role.id })}
            >
              运行{role.name}会审
              {project.mode === 'live' ? '（模型调用）' : '（演示）'}
            </Button>
            {report && (
              <>
                <p>
                ...
                {report.findings.map((f, i) => (
                  <div className="issue" key={i}>
                    <div>
                      ...
                      <small>
                        返工岗位：
                        {filmTeam.find((r) => r.id === f.returnTo)?.name}
                      </small>
                    </div>
                  </div>
                ))}
