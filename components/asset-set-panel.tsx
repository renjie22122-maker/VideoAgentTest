...
export function AssetSetPanel({source,library,busy,act}:{source:Asset;library:Asset[];busy:boolean;act:(action:string,data?:Record<string,unknown>)=>Promise<unknown>}){
 ...
 {kind==='character'&&source.approved&&<Button disabled={busy} onClick={()=>void act('asset_use_costume',{assetId:source.id})}>使用此套服装（更新关联镜头素材）</Button>}{source.costumeOf&&<p className="help">这是独立服装方案，需要明确选择后才用于分镜，不会与原服装同时输入。</p>}
 </section>;
}
