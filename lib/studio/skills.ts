...
export const productionSkills = {
  ...
  director: { name: '导演分镜与摄影调度', role: '导演', version: '4.0.0' },
  continuity: { name: '场记与叙事连续性', role: '场记', version: '2.0.0' },
...
export function skillGuide(id: ProductionSkillId, p?: Project): string {
  const stage: Record<ProductionSkillId, NodeId> = {
    clarify: 'clarify',
    writer: 'script',
    assets: 'assets',
    director: 'storyboard',
  ...
  return (
    ...
    (p
      ? '\n项目部门责任：\n' +
        projectAgents(p)
          .filter((r) => r.enabled && r.stages.includes(stage[id]))
          .map((r) => r.name + '：' + r.deliverable + '；' + r.checks)
          .join('\n')
      : teamInstructions(stage[id]))
  );
}
export function skillCatalog() {
  return Object.entries(productionSkills).map(([id, meta]) => ({
    ...meta,
    id,
    instructions: skillGuide(id as ProductionSkillId),
  }));
}
