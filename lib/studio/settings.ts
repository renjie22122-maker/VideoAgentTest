...
export const settingKeys=['LLM_BASE_URL','LLM_API_KEY','LLM_MODEL','MEDIA_GATEWAY_URL','MEDIA_API_KEY','IMAGE_MODEL','VIDEO_MODEL','QA_GATEWAY_URL','QA_API_KEY','IMAGE_PROVIDER','OPENAI_IMAGE_MODEL','OPENAI_IMAGE_API_KEY','FAL_API_KEY'] as const;
...
export function saveSettings(input:unknown):SettingsView {
 ...
 for(const [k,v] of Object.entries(fields)){
   ...
   if(key.endsWith('_URL')&&value){let url:URL;try{url=new URL(value);}catch{throw new Error('请填写有效的 API 地址。');}
     ...
     if(url.protocol!=='https:'&&!(url.protocol==='http:'&&['localhost','127.0.0.1'].includes(url.hostname)))throw new Error('API 地址必须使用 HTTPS，本机服务可使用 HTTP。');
 ...
 for(const [urlKey,key] of [['LLM_BASE_URL','LLM_API_KEY'],['MEDIA_GATEWAY_URL','MEDIA_API_KEY'],['QA_GATEWAY_URL','QA_API_KEY']] as const){
   const old=data.values[urlKey]??process.env[urlKey]??'',next=values[urlKey]??old;
