...
export function shotReferences(p:Project,shotId:string,limit=10){
 ...
 const candidates=approvedAssets(p,shotId).filter(a=>a.kind!=='prop'||text.includes(a.name)).sort((a,b)=>{
  const score=(v:typeof a)=>v.kind==='character'?(text.includes(v.name)?0:2):v.kind==='background'?1:3;
  return score(a)-score(b);
 });
 const entries=candidates.map(a=>({url:a.url!,name:a.name,kind:a.kind,version:a.version}));
