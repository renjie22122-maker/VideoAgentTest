...
export function requeueFailedJob(job:Job,maxRetries:number){
 ...
 const localFailure=/最多使用 10 张参考图|上一镜头尚未成功|旧版资产提示词/.test(job.error??'');
 if(job.remoteId==='fal-pending'&&!localFailure)throw new Error('第 '+job.shotId+' 镜上次提交结果未知，不能直接重发。请先核查 fal 记录；已出图可上传到该镜头。无需修改分镜来绕过限制。');
