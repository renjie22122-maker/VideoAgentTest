...
export const DIRECTOR_SCHEMA = MOTION_SCHEMA+`只输出 {"shots":[...]}。每镜必填 scene（剧本场次 ID，不是地点名称）、title（1–100 字）、beat（1–1000 字）、description（1–4000 字）、dialogue（字符串）、sound（字符串）、duration（2–15 秒的数字）、size（wide/medium/close）、transition（cut/dissolve）、camera、startState、endState。
...
按 sceneTiming 顺序拆分每个场次，每场镜头时长合计与该场秒数一致，全片合计等于 duration。不得通过跳过场次或截断动作来凑时长。`;

export function validateDirectorPlan(value:unknown,p:Project):Plan {
 ...
 if(scenes?.length){
  ...
  for(const [i,shot] of shots.entries()){
   ...
   if(index<previous)throw new Error('第 '+(i+1)+' 镜的场次顺序与已确认剧本不一致。');
   previous=index;
  }
