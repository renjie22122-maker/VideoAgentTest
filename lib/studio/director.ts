...
export function validateDirectorPlan(value:unknown,p:Project,requireIntent=false):Plan {
 ...
 if(!Array.isArray(raw.shots)||raw.shots.length<1)throw new Error('shots 必须包含 至少 1 个镜头。');
 ...
 const plan=validatePlan({title:script?.title??raw.title??fallback.title,logline:script?.logline??raw.logline??fallback.logline,synopsis:script?.synopsis??raw.synopsis??fallback.synopsis,bible:p.production?.assets?.bible??raw.bible??fallback.bible,shots});
 if(requireIntent){const errors=buildQualityReport({...p,plan}).findings.filter(f=>f.severity==='error');if(errors.length)throw new Error('分镜专业预检未通过：'+errors.slice(0,8).map(f=>f.shotIds.join('/')+' '+f.message+'；依据：'+f.evidence).join('；'));}
 return plan;
}
