    ...
    p.mode === 'demo'
      ...
      : await roleJSON(
          ...
          skillGuide('continuity', p) + '\n交付：' +
            ...
            '。' +
            role.checks +
            ' 只审查提供的文本，不声称看过图片或视频。同一资产有多个候选、仅一个批准版本是正常状态，不得报告为批准不一致；按 familyId、viewId 和 costumeOf 区分版本及套组。返回 {summary:string,findings:[{shotId:string, severity:"note"|"warning"|"error", evidence:string, suggestion:string, returnTo:string}]}。全局问题 shotId 为空字符串。returnTo 必须是给定岗位 ID。最多 20 项，证据不足则注明需要人工确认。verification 若存在，这是对修订后当前版本的独立复核：逐条对照 previousFindings 与当前分镜，已经解决的问题不沿用旧结论；未解决或新问题必须引用当前文本证据并给出返工岗位。不得因另一岗位声称已修复就返回空报告。遵循 assetPolicy：未生成建议或可选参考图只记 note，不能当作缺剧情资产；允许按已确认文字设计自由生成。资产外观与剧本明确事实冲突仍须报告。依据各叙事线追踪状态，允许有标注的交叉剪辑与跨镜声音；输出评估只覆盖文本，不是实际视频质量合格证书。',
          {
            idea: p.idea,
            brief: p.brief,
