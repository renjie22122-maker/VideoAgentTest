...
import { assetRequirementLabels } from './asset-policy.ts';
...
export function validateAssetDesigns(raw:Record<string,unknown>,p:Project,options:{sources?:string[];allowEmpty?:boolean}={}):Asset[]{
 if(!Array.isArray(raw.assets)||(!raw.assets.length&&!options.allowEmpty)||raw.assets.length>24)throw new Error('资产清单需要 1–24 项。');
 ...
 collect(p.production?.script);collect(p.production?.assets?.bible);collect(p.plan?.shots.map(s=>({description:s.description,start:s.startState,end:s.endState,intent:s.intent})));collect(options.sources);
 ...
 return raw.assets.map(value=>{
  ...
  if(!sources.some(source=>source.includes(evidence)))throw new Error('资产「'+name+'」的依据未匹配剧本、美术设定或分镜原文：'+JSON.stringify(evidence)+'。请从 script、bible 或 storyboard 的单个文本字段逐字引用连续片段，不要改写、拼接或引用任务要求。');
  if(seen.has(kind+name))throw new Error('资产名称重复。');seen.add(kind+name);
  const requirement=v.requirement??(kind==='character'?'required':'recommended');
  if(typeof requirement!=='string'||!Object.hasOwn(assetRequirementLabels,requirement))throw new Error('资产等级无效。');
  if(v.sceneIds!==undefined&&(!Array.isArray(v.sceneIds)||v.sceneIds.some(id=>typeof id!=='string'||!p.production?.script?.scenes?.some(s=>s.id===id))))throw new Error('资产关联了不存在的场次。');
  const sceneIds=v.sceneIds===undefined?undefined:[...new Set(v.sceneIds as string[])];
  const requirementReason=v.requirementReason===undefined?undefined:text(v.requirementReason,'等级说明',400);
  ...
  return {id:randomUUID(),name,kind,design,evidence,requirement:requirement as Asset['requirement'],requirementReason,sceneIds,designOrigin:'agent',promptVersion:ASSET_PROMPT_VERSION,prompt:compileAssetDesign(kind,name,design),referenceIds:[],version:1,approved:false,status:'draft',createdAt:Date.now()};
 });
}
