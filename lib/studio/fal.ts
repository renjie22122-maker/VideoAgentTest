...
export async function submitFal(prompt:string,refs:string[],compose=false){
 ...
 const data=await call('https://queue.fal.run/'+model,{prompt,...(images.length?{image_urls:images}:{}),num_images:1});
...
export async function pollFal(remoteId:string):Promise<{status:'running'|'succeeded'|'failed';outputUrl?:string;error?:string}>{
 const task=JSON.parse(remoteId.slice(4)) as {status:string;result:string};const status=await call(task.status);
 ...
 const result=await call(task.result);if(result.error)return {status:'failed',error:'fal 未能生成图片。'};
